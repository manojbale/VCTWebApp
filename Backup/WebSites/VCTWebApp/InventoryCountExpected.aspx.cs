...
{
    public partial class InventoryCountExpected : Microsoft.Practices.CompositeWeb.Web.UI.Page//, IRegionalOfficeRequestView
    {
        ...
        protected void ddlHospital_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                PopulateExpectedInventoryCount();
            }
            catch (Exception ex)
            {
                //this.lblError.Text = string.Format(CultureInfo.InvariantCulture, vctResource.GetString(ex.Message), this.lblHeader.Text);
        ...
        private void PopulateExpectedInventoryCount()
        {
            gdvInventoryCount.DataSource = null;
            if (ddlHospital.SelectedIndex != 0)
            {
                gdvInventoryCount.DataSource = new PartyRepository().GetExptectedInventoryCountByPartyId(Convert.ToInt64(ddlHospital.SelectedValue));
            }
