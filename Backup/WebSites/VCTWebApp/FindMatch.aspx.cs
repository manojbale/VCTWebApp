...
{
    public partial class FindMatch : Microsoft.Practices.CompositeWeb.Web.UI.Page, IFindMatchView
    {
        #region Instance Variables

        private FindMatchPresenter _presenter;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    if (Session["RequestIdToSearch"] != null)
                    {
        ...
        [CreateNew]
        public FindMatchPresenter Presenter
        {
            get
            {
        ...
        public long SelectedRequestId
        {
            get
            {
                if (Session["RequestIdToSearch"] != null)
                {
                    return Convert.ToInt64(Session["RequestIdToSearch"]);
                }
                else
                    return 0;
            }
            ...
            try
            {
                if (e.CommandName == "SendRequest")
                {
                    if (String.IsNullOrEmpty(hdnStatus.Value))
                    {
                        ...
                        Constants.ResultStatus resultStatus = this.Presenter.SendRequest();
                        if (resultStatus == Constants.ResultStatus.RequestSent)
                        {
                            this.lblError.Text = "<font color='blue'>" + string.Format(CultureInfo.InvariantCulture, vctResource.GetString("msgSentForConfirmation")) + "</font>";
        ...
        private void AuthorizedPage()
        {
            ...
            else if (security.HasAccess("PendingRequests"))
            {
                CanView = security.HasPermission("PendingRequests.View");
            }
            else
